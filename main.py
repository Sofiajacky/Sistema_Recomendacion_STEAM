...
def get_developer_stats(desarrollador: str):
    ...
    df = pd.read_parquet('Data_Clean/games_clean.parquet')
...
#    money_spent = float(money_spent) if not pd.isnull(money_spent) else 0.0  
#    recommendation_percentage = float(recommendation_percentage) if not pd.isnull(recommendation_percentage) else 0.0  
...
#    horas_por_año['Año'] = horas_por_año['Año'].astype(int)
...
#df_merged = pd.read_parquet(r'C:\Users\Sofi\Desktop\Henry\LABS\Proyecto_STEAM\Consultas_API\df_merged.parquet')
